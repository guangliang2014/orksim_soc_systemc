		   ...
		   void      *p )
{
  ...
  for( Fd2Inst *cur = instList; cur != NULL ; cur = cur->next ) {
    if( FD_ISSET( cur->fd, &readFdSet )) {
      // std::cout << "notify "<< sc_core::sc_delta_count() << std::endl;
      (cur->inst)->ioEvent->notify(sc_core::SC_ZERO_TIME);
    }
  }
}	// ioHandler()
