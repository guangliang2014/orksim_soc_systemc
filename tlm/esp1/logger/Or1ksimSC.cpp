...
{
  std::string config_string(configFile);
  std::string image_string(imageFile);

  char *sim = "sim";
  char *opt = "-f";
  char *config_s = &config_string[0];
  char *image_s = &image_string[0];
  char *argv[] = {sim, opt, config_s, image_s};

  or1ksim_init(4, argv, this, staticReadUpcall, staticWriteUpcall);
